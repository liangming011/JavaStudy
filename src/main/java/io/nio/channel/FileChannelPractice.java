...
public class FileChannelPractice {
    ...
    public static void main(String[] args) {
        try {
            ...
            RandomAccessFile aFile0 = new RandomAccessFile("data.txt", "rw");
            FileChannel channel = aFile0.getChannel();
            ByteBuffer buf0 = ByteBuffer.allocate(48);
            buf0.clear();
            buf0.put(newData.getBytes());
            ...
            /**
             * 如果将位置设置在文件结束符之前，然后试图从文件通道中读取数据，读方法将返回-1 —— 文件结束标志。
             *
             ...
             * 操作系统会将数据缓存在内存中，所以无法保证写入到FileChannel里的数据一定会即时写到磁盘上。要保证这一点，需要调用force()方法。
             *
