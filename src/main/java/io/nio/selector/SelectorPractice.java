...
public class SelectorPractice {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            SelectionKey selectionKey = channel.register(selector,SelectionKey.OP_READ);//为通道开启读模式
            ...
            //如果你对不止一种事件感兴趣，那么可以用“位或”操作符将常量连接起来，如下：在下面还会继续提到interest集合。
            int interestSet0 = SelectionKey.OP_READ | SelectionKey.OP_WRITE;
            ...
            * */
             ...
             */
            int interestSet1 = selectionKey.interestOps();

            boolean isInterestedInAccept  = (interestSet1 & SelectionKey.OP_ACCEPT) == SelectionKey.OP_ACCEPT;
            boolean isInterestedInConnect = (interestSet1 & SelectionKey.OP_CONNECT) == SelectionKey.OP_CONNECT;
            boolean isInterestedInRead    = (interestSet1 & SelectionKey.OP_READ) == SelectionKey.OP_READ;
            boolean isInterestedInWrite   = (interestSet1 & SelectionKey.OP_WRITE) == SelectionKey.OP_WRITE;
             ...
             * */
            Channel  channel0  = selectionKey.channel();
            Selector selector0 = selectionKey.selector();
            ...
            //通过Selector选择通道
            //一旦向Selector注册了一或多个通道，就可以调用几个重载的select()方法。这些方法返回你所感兴趣的事件
            // （如连接、接受、读或写）已经准备就绪的那些通道。换句话说，如果你对“读就绪”的通道感兴趣，select()方法会返回读事件已经就绪的那些通道。
            /**
             * 下面是select()方法：
             *
             * int select() : select()阻塞到至少有一个通道在你注册的事件上就绪了。
             * int select(long timeout) : select(long timeout)和select()一样，除了最长会阻塞timeout毫秒(参数)。
             * int selectNow() : selectNow()不会阻塞，不管什么通道就绪都立刻返回（译者注：此方法执行非阻塞的选择操作。
            ...
            while(keyIterator.hasNext()) {
                ...
                if(key.isAcceptable()) {
                    ServerSocketChannel sadda = (ServerSocketChannel) key.channel();
            ...
            /**
             * 完整的示例
             * 这里有一个完整的示例，打开一个Selector，注册一个通道注册到这个Selector上(通道的初始化过程略去),
             * 然后持续监控这个Selector的四种事件（接受，连接，读，写）是否就绪。
             * */
            Selector selector1 = Selector.open();
            channel.configureBlocking(false);
            SelectionKey key3 = channel.register(selector1, SelectionKey.OP_READ);
            while(true) {
                ...
                Set selectedKeys1 = selector1.selectedKeys();
                Iterator keyIterator1 = selectedKeys1.iterator();
                while(keyIterator1.hasNext()) {
                    SelectionKey key2 =  (SelectionKey) keyIterator1.next();
