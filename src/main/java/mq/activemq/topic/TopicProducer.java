...
import mq.activemq.queue.QueueProducer;
...
public class TopicProducer {
    ...
    // 消息请求目的地（对于点对点模型，是Queue对象；对于发布订阅模型，是Topic对象；它们都继承或实现了该接口）
    ...
    public void doSend() {
        try {
            ...
            responseDestination = session.createTopic(RESPONSE_NAME);
            /**
             * 6.创建一个消息生产者，并指定目的地
             */
                    ...
                    public void onMessage(Message message) {
                        if (message instanceof TextMessage) {
                            try {
                                ...
                                if (message != null && message.getJMSCorrelationID().equals(message.getJMSCorrelationID())) {
                                    System.out.println("响应:“" + msg.getText() + "”");
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
