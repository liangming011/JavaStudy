package reflect;

import java.lang.reflect.Method;

/*
 * 通过反射获取成员方法并调用：
 *
 *   1.批量的：
 *       public Method[] getMethods():获取所有“公有方法”；（包含了父类的方法也包含Object类）
 *       public Method[] getDeclaredMethods():获取所有的成员方法，包括私有的（不包括继承的）
 *   2.获取单个的
 *       public Method[] getMethod(String name,Class<?>... parameterTypes):
 *               参数：
 *                   name ： 方法名；
 *                   Class ... : 形参的Class类型对象
 *       public Method[] getDeclaredMethod(String name,Class<?>... parameterTypes);
 *
 *     调用方法：
 *       Method -->public Object invoke(Object obj,Object... args);
 *               参数说明：
 *                   obj : 要调用方法的对象；
 *                   args ： 调用方法是所传递的参数
 * */
public class Reflection005 {

    public static void main(String[] args) throws Exception {
        //获取Class对象
        Class clazz = Class.forName("reflect.Teacher");

        //获取所有的公有的成员方法
        System.out.println("*********************获取所有公有的成员方法**********************");
        Method[] mArray = clazz.getMethods();
        for (Method m : mArray) {
            System.out.println(m);
        }

        //获取所有的成员方法
        System.out.println("***********获取所有的成员方法(包括私有的，受保护的，默认的)*********");
        mArray = clazz.getDeclaredMethods();
        for (Method m : mArray) {
            System.out.println(m);
        }

        //获取某个公有的成员方法
        System.out.println("*********************获取某个公有的成员方法**********************");
        Method m = clazz.getMethod("show1", String.class);
        System.out.println(m);

        //实例化一个teacher对象
        Object obj = clazz.getConstructor().newInstance();
        m.invoke(obj, "tony");

        //获取某个成员方法
        System.out.println("*********************获取某个成员方法**********************");
        m = clazz.getDeclaredMethod("show2", String.class);
        System.out.println(m);

        //实例化一个teacher对象
        m.setAccessible(true);//解除私有限定
        //获取要调用的对象
        obj = clazz.getConstructor().newInstance();

        Object res = m.invoke(obj, "tony");//需要两个参数，一个是调用的对象（获取有反射），一个是实参
        System.out.println("返回值：" + res);

    }
}
